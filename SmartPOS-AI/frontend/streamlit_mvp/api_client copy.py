...
def _post(path, json=None, files=None, params=None, data=None, token=None):
    try:
...
def get_transaction_history(days: int = 30) -> pd.DataFrame:
    df = pd.DataFrame(_get("/transactions/history", params={"days": days}))
    if not df.empty:
