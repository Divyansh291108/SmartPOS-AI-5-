...
def load_history(days=30):
    return api.get_transaction_history(days=days)

@st.cache_data(ttl=5, show_spinner=False)
def load_dashboard_summary():
    return api.get_dashboard_summary()
...
if page == "📊 Dashboard":
    ...
    st.caption("Real-time visibility across payments, sales, and stock health.")
    ...
    with c1: metric_card("Today's Revenue", f"₹{summary['revenue']:,.0f}", f"{summary['transaction_count']} transactions")
    ...
    with c5: metric_card("Today's Profit", f"₹{summary['profit']:,.0f}")
    ...
    with left:
        ...
        if hist.empty:
            ...
            fig = px.area(daily, x="date", y="revenue", labels={"revenue": "Revenue (₹)", "date": "Date"})
            fig.update_traces(line_color=PRIMARY, fillcolor="rgba(37,99,235,0.15)")
            fig.update_layout(height=320, margin=dict(l=10, r=10, t=10, b=10))
            st.plotly_chart(fig, width='stretch')

            st.subheader("Payment Method Split (30 days)")
            pay_split = hist.groupby("payment_method")["revenue"].sum().reset_index()
            fig2 = px.pie(pay_split, names="payment_method", values="revenue", hole=0.55,
...
# ----------------------------------------------------------------------------
    ...
    """with col_cart:
        ...
        if not st.session_state.cart:
            ...
            for pid, qty in st.session_state.cart.items():
                ...
                cart_rows.append((pid, p["name"], p["price"], qty, line_total))

            for pid, name, price, qty, line_total in cart_rows:
                c1, c2, c3 = st.columns([3, 2, 1])
                c1.markdown(f"**{name}**  \n₹{price:.0f} × {qty}")
                new_qty = c2.number_input("Qty", min_value=0, max_value=999, value=int(qty),
                ...
                if new_qty != qty:
                    ...
                    st.rerun()
                c3.markdown(f"₹{line_total:.0f}")

            discount_pct = st.slider("Discount % (applied to whole cart)", 0, 50, 0)
            discounted_total = total * (1 - discount_pct / 100)
            gst_after_discount = total_gst_embedded * (1 - discount_pct / 100)
            taxable_after_discount = discounted_total - gst_after_discount

            st.markdown("---")
            st.markdown(f"Taxable value: ₹{taxable_after_discount:,.2f}")
            st.markdown(f"GST (included in price): ₹{gst_after_discount:,.2f}")
            if discount_pct:
                st.markdown(f"Subtotal: ~~₹{total:,.0f}~~")
            st.markdown(f"### Total: ₹{discounted_total:,.0f}")

            payment_method = st.radio("Payment Method", ["Cash", "UPI", "Card", "Split"], horizontal=True)

            upi_confirmed = True
            if payment_method == "UPI":
                if not SHOP_UPI_ID:
                    st.warning("No UPI ID configured for this shop — set SMARTPOS_SHOP_UPI_ID to enable this.")
                    upi_confirmed = False
                else:
                    preview_ref = f"PREVIEW{len(st.session_state.cart)}"
                    uri = upi_qr.build_upi_uri(SHOP_UPI_ID, SHOP_NAME, discounted_total, "SmartPOS Sale", preview_ref)
                    qr_png = upi_qr.generate_qr_png(uri)
                    st.image(qr_png, caption=f"Scan to pay ₹{discounted_total:,.2f}", width=220)
                    upi_confirmed = st.checkbox("✅ Payment received — confirmed on my phone")

            if payment_method == "Split":
                split_cash = st.number_input("Cash portion (₹)", min_value=0, max_value=int(discounted_total), value=int(discounted_total // 2))
                st.caption(f"UPI/Card portion (auto): ₹{max(0, discounted_total - split_cash):,.0f}")
                st.caption("Note: the backend records one payment method per checkout — split payments are logged as Cash for now.")

            if st.button("✅ Checkout", type="primary", use_container_width=True,
                         disabled=(payment_method == "UPI" and not upi_confirmed)):
                items = [{"product_id": pid, "qty": qty, "discount_pct": discount_pct} for pid, _, _, qty, _ in cart_rows]
                pm_to_send = "Cash" if payment_method == "Split" else payment_method
                result = call_authed(api.checkout, items, pm_to_send, token=token)
                if result is not None:
                    st.session_state.cart = {}
                    refresh_all()
                    st.success(f"Transaction {result['txn_id']} completed — ₹{result['total']:,.0f} via {payment_method}")
                    if result.get("total_tax"):
                        st.caption(
                            f"GST breakdown: taxable value ₹{result['taxable_value']:,.2f} · "
                            f"CGST ₹{result['cgst']:,.2f} · SGST ₹{result['sgst']:,.2f} · "
    ...
    if not tx_df.empty:
        summary_tbl = tx_df.groupby(["txn_id", "cashier", "payment_method"]).agg(
...
# ----------------------------------------------------------------------------
    ...
    with tab_users:
        ...
        with st.form("create_user_form"):
            ...
            new_role = st.selectbox("Role", ["cashier", "owner"])
