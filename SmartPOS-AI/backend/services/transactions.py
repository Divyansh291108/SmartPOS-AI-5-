...
def get_transaction_history(db: Session, days: int = 30, start_date: date = None, end_date: date = None):
    """Raw transaction lines for charting. Pass start_date+end_date for an
    explicit range (used by the Dashboard's date picker); otherwise falls
    back to the last `days` days, same as before."""
    if start_date and end_date:
        since = datetime.combine(start_date, datetime.min.time())
        until = datetime.combine(end_date, datetime.max.time())
        return (
            db.query(Transaction)
            .filter(Transaction.timestamp >= since, Transaction.timestamp <= until)
...
def get_dashboard_summary(db: Session, start_date: date = None, end_date: date = None) -> dict:
    if start_date and end_date:
        since = datetime.combine(start_date, datetime.min.time())
        until = datetime.combine(end_date, datetime.max.time())
        txns = (
        ...
        txns = get_todays_transactions(db)

    revenue = sum(t.revenue for t in txns)
    profit = sum(t.profit for t in txns)
    cash = sum(t.revenue for t in txns if t.payment_method == "Cash")
    upi = sum(t.revenue for t in txns if t.payment_method == "UPI")
    card = sum(t.revenue for t in txns if t.payment_method == "Card")
    txn_count = len(set(t.txn_id for t in txns))
